	...
	return &pluginsdk.Resource{
		...
		DeprecationMessage: func() string {
			if features.ThreePointOhBeta() {
				return "The `azurerm_sql_virtual_network_rule` resource is deprecated and will be removed in version 4.0 of the AzureRM provider. Please use the `azurerm_mssql_virtual_network_rule` resource instead."
			}
			return ""
		}(),
