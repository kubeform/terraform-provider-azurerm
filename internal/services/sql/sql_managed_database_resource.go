	...
	return &schema.Resource{
		...
		DeprecationMessage: func() string {
			if features.ThreePointOhBeta() {
				return "The `azurerm_sql_managed_database` resource is deprecated and will be removed in version 4.0 of the AzureRM provider. Please use the `azurerm_mssql_managed_database` resource instead."
			}
			return ""
		}(),
